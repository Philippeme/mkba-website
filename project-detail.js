...
function initProjectDetailScrollAnimations() {
    ...
    if (metadata) {
        ...
        metadata.style.transform = 'translateY(30px)';
        metadata.style.transition = 'all 0.8s ease-out 0.3s';
        metadataObserver.observe(metadata);
    }
    
    // Animation de la section de téléchargement
    ...
    if (downloadSection) {
        ...
        downloadSection.style.transition = 'all 0.6s ease-out 0.6s';
    ...
    const projectImages = document.querySelectorAll('.project-image, .contextual-image');
    projectImages.forEach((image, index) => {
        ...
        image.style.transition = 'all 0.7s cubic-bezier(0.25, 0.46, 0.45, 0.94)';
        imageObserver.observe(image);
    });
    
    // Animation des cartes projets similaires
    const similarProjectCards = document.querySelectorAll('.similar-project-card');
    similarProjectCards.forEach((card, index) => {
